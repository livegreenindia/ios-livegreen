...
import { Users, Building2, MapPin, TrendingUp, Clock, CheckCircle } from 'lucide-react';
...
interface RecentActivity {
  id: string;
  type: 'user' | 'club' | 'place' | 'completion';
  title: string;
  timestamp: Date;
}
...
export default function DashboardPage() {
  ...
  const loadDashboardData = async () => {
    try {
      ...
      // Get pending clubs
      const pendingClubsQuery = query(
        collection(db, 'clubs'),
        where('status', '==', 'pending')
      );
      const pendingClubsSnap = await getDocs(pendingClubsQuery);
      const pendingClubs = pendingClubsSnap.size;

      // Get pending places
      const pendingPlacesQuery = query(
      ...
      const activities: RecentActivity[] = recentUsersSnap.docs.map(doc => ({
        id: doc.id,
        type: 'user' as const,
        title: `New user: ${doc.data().email || doc.data().name || 'Unknown'}`,
